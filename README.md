...
* [`src/covid19_spread_simulation/`](src/covid19_spread_simulation/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/covid19_spread_simulation/core.cljs): contains the SPA entry point, `init`
* [`test/covid19_spread_simulation/`](test/covid19_spread_simulation/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `covid19-spread-simulation` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/covid19_spread_simulation/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns covid19-spread-simulation.example
  (:require [covid19-spread-simulation.config :as config])
